...
static esp_err_t index_handler(httpd_req_t *req){
    ...
    page += "<title>Beatrix</title>";
    page += "<style>";
    page += "body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #2c3e50; color: white; text-align: center; margin: 0; padding: 20px; }";
    page += "h2 { margin-top: 10px; color: #ecf0f1; letter-spacing: 1px; }";
    page += ".btn { background-color: #3498db; color: white; border: none; border-radius: 12px; width: 90px; height: 80px; font-size: 24px; font-weight: bold; margin: 5px; box-shadow: 0 5px #2980b9; cursor: pointer; transition: 0.1s; }";
    page += ".btn:active { transform: translateY(4px); box-shadow: 0 1px #2980b9; }";
    page += ".btn-stop { background-color: #e74c3c; box-shadow: 0 5px #c0392b; font-size: 16px; }"; 
    page += ".btn-stop:active { box-shadow: 0 1px #c0392b; }";
    page += ".btn-light { background-color: #f1c40f; color: #333; width: 140px; height: 50px; border-radius: 25px; box-shadow: 0 5px #f39c12; font-size: 16px; font-weight: bold; }";
    page += ".btn-light:active { box-shadow: 0 1px #f39c12; }";
    page += ".stream-box { border: 4px solid #34495e; border-radius: 10px; box-shadow: 0 8px 16px rgba(0,0,0,0.5); width: 300px; height: 300px; transform: rotate(90deg); margin: 30px auto; display: block; }";
    ...
    page += "<h2>Beatrix</h2>";
    page += "<IMG SRC='http://" + WiFiAddr + ":81/stream' class='stream-box'><br/><br/>";
    
    page += "<div><button class='btn' onmousedown=getsend('go') onmouseup=getsend('stop') ontouchstart=getsend('go') ontouchend=getsend('stop')>^</button></div>";
    
    page += "<div>";
    page += "<button class='btn' onmousedown=getsend('left') onmouseup=getsend('stop') ontouchstart=getsend('left') ontouchend=getsend('stop')>&lt;</button>";
    page += "<button class='btn btn-stop' onmousedown=getsend('stop') onmouseup=getsend('stop')>STOP</button>";
    page += "<button class='btn' onmousedown=getsend('right') onmouseup=getsend('stop') ontouchstart=getsend('right') ontouchend=getsend('stop')>&gt;</button>";
    page += "</div>";
    
    page += "<div><button class='btn' onmousedown=getsend('back') onmouseup=getsend('stop') ontouchstart=getsend('back') ontouchend=getsend('stop')>⌄</button></div><br/>";

    page += "<div>";
    page += "<button class='btn btn-light' onmousedown=getsend('ledon')>Flash</button> &nbsp;";
    page += "<button class='btn btn-light' onmousedown=getsend('ledoff')>OFF</button>";
    page += "</div>";
